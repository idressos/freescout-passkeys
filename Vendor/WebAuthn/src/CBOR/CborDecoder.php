 ...
 */
class CborDecoder {
    // --- Local modification for the FreeScout Passkeys module -------------
    // Upstream imposes no recursion-depth limit, so a maliciously nested CBOR
    // payload can drive _parseItem() into deep recursion / large allocations.
    // MAX_DEPTH bounds the nesting far above anything a real WebAuthn message
    // uses (attestation objects nest ~3 levels). This is the only change to
    // the upstream file.
    const MAX_DEPTH = 32;
    // ---------------------------------------------------------------------
    ...
    protected static function _parseItem(ByteBuffer $buf, &$offset, $depth = 0) {
        // Local modification (FreeScout Passkeys): bound recursion depth.
        if ($depth > self::MAX_DEPTH) {
            throw new WebAuthnException('Maximum CBOR nesting depth exceeded.', WebAuthnException::CBOR);
        }
